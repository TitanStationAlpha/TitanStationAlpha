	...
	switch(action)
		if("Reset")
			if(target.wires)
				...
				target.visible_message("You hear a silent click.")
		if("Add X-Ray")
			if(target.isXRay())
...
/datum/game_mode/malfunction/verb/elite_encryption_hack()
	...
	set desc = "200 CPU - Allows you to hack station's ALERTCON system, changing alert level."
	var/price = 200
	var/mob/living/silicon/ai/user = usr
	if(!ability_prechecks(user, price))
		return

	var/alert_target = input("Select new alert level:") in list("green", "blue", "red", "delta")
